...
interface GetOfertaPorEmpresaResponse {
  obtenerOfertasPorEmpresa: OfertaTrabajoType[];
}
...
export const ofertaTrabajoService = {
  async getOfertasTrabajo(limit: number = 10): Promise<OfertaTrabajoType[]> {
    try {
      ...
      const result = await executeQuery<GetOfertasResponse>(OfertaQueries.GET_OFERTAS_TRABAJO, { limit });
  ...
  async getOfertaTrabajoPorId(id: string): Promise<OfertaTrabajoType | null> {
    try {
      ...
      const result = await executeQuery<GetOfertaPorIdResponse>(OfertaQueries.GET_OFERTA_TRABAJO_POR_ID, { id });
  ...
  async getOfertaTrabajoPorEmpresa(empresaId: string): Promise<OfertaTrabajoType[]> {
    try {
      console.log(`\n📋 SERVICE: Fetching ofertas por empresa with id: ${empresaId}`);

      if (!empresaId || empresaId.trim() === "") {
        throw new Error("Invalid empresa ID");
      }

      const result = await executeQuery<GetOfertaPorEmpresaResponse>(OfertaQueries.GET_OFERTA_TRABAJO_POR_EMPRESA, {
        empresaId,
      });

      if (!result?.obtenerOfertasPorEmpresa) {
        console.warn(`⚠️ Ofertas para empresa ${empresaId} not found`);
        return [];
      }
      console.log("✅ Ofertas fetched:", result.obtenerOfertasPorEmpresa);
      return result.obtenerOfertasPorEmpresa;
    } catch (error) {
      console.error(`❌ Error fetching ofertas por empresas ${empresaId}:`, error);
      throw error;
    }
  },
  ...
  async crearOfertaTrabajo(data: {
    ...
    try {
      ...
      const result = await executeMutation<CreateOfertaResponse>(OfertaMutations.CREAR_OFERTA_TRABAJO, data);
  ...
  async actualizarOfertaTrabajo(data: {
    ...
    try {
      ...
      const result = await executeMutation<UpdateOfertaResponse>(OfertaMutations.ACTUALIZAR_OFERTA_TRABAJO, data);
  ...
  async eliminarOfertaTrabajo(id: string): Promise<string> {
    try {
      ...
      const result = await executeMutation<DeleteOfertaResponse>(OfertaMutations.ELIMINAR_OFERTA_TRABAJO, { id });
