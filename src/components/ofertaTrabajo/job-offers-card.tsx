...
export function JobOffersCard({ offer, onApply }: JobOffersCardProps) {
  ...
  const fechaFormato = new Date(offer.fechaPublicacion).toLocaleDateString("es-ES", {
    year: "numeric",
    month: "short",
    day: "numeric",
  });
  ...
  return (
    <div className="group h-full bg-white border-2 border-gray-200 rounded-2xl overflow-hidden hover:shadow-2xl hover:border-blue-400 transition-all duration-300 flex flex-col hover:scale-105 transform">
      ...
      <div className="bg-linear-to-r from-blue-600 via-blue-500 to-purple-600 px-6 py-5">
        ...
        <h3 className="text-xl font-bold text-white mb-1 leading-tight">{offer.titulo}</h3>
        <p className="text-blue-100 font-medium text-sm">{offer.empresa?.nombre || "Empresa desconocida"}</p>
      </div>

      {/* Descripción */}
      <div className="px-6 pt-5">
        <p className="text-sm text-gray-600 mb-5 line-clamp-2 leading-relaxed">{offer.descripcion}</p>
      </div>
      ...
      <div className="px-6 space-y-4 mb-6 grow">
        ...
        {offer.salario && (
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-sm font-bold text-green-600">${offer.salario.toLocaleString("es-ES")}</span>
            </div>
          </div>
        )}
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <span className="text-sm text-gray-700 line-clamp-2">{offer.requisitos}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          <div className="flex items-center gap-2 bg-gray-50 rounded-lg p-3">
            ...
            <div>
              ...
              <span className="text-sm font-bold text-gray-800">{postulacionesCount}</span>
            </div>
          </div>
          <div className="flex items-center gap-2 bg-gray-50 rounded-lg p-3">
      ...
      {/* Email de Contacto */}
      <div className="mx-6 mb-4 p-4 bg-linear-to-br from-blue-50 to-purple-50 border-2 border-blue-200 rounded-xl">
        <p className="text-xs text-blue-700 font-bold mb-2 uppercase tracking-wider">📧 Contacto</p>
        <p className="text-sm font-semibold text-gray-800 break-all">{offer.empresa?.correo || "No disponible"}</p>
      </div>
      ...
      <div className="px-6 pb-6">
        <button
          onClick={onApply}
          className="w-full bg-linear-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-bold py-3 rounded-xl shadow-lg hover:shadow-2xl transform hover:scale-105 transition-all duration-300 flex items-center justify-center gap-2"
        >
